...
pub fn collect() -> Result<PressureReport> {
    ...
    let memory   = fs::read_to_string("/proc/pressure/memory").context("read /proc/pressure/memory")?;
    ...
    Ok(PressureReport { cpu_some, memory_some, memory_full, io_some, io_full })
}
...
fn extract_f64(line: &str, key: &str) -> Result<f64> {
    let start = line.find(key).with_context(|| format!("key '{key}' not found"))? + key.len();
    let rest = &line[start..];
    let end = rest.find(' ').unwrap_or(rest.len());
    rest[..end].parse::<f64>().with_context(|| format!("parse f64 for '{key}'"))
}
